...
VALUES
  ('live_stream_info', 'Прямий ефір', 'Доступ до трансляції відкривається після підтвердження донату адміністратором. Слідкуйте за нашими новинами та актуальними зборами.'),
  ('volunteer_contacts', 'Координація волонтерів', 'Напишіть координатору у Telegram: @volonterka_help або телефонуйте +380931234567.'),
  ('home_reviews_intro', 'Голоси донорів', 'Кожне добре слово мотивує команду працювати ще більше.');
...
VALUES
  ('Як ми готуємо пікапи до фронту', 'Розповідаємо про процес пошуку, ремонту та передачі автомобілів для захисників.', 'Повний матеріал про те, як команда фонду працює з автомобілями: від закупівлі до передачі на передову. Ми ділимося чек-листом перевірки та списком партнерів, які допомагають із ремонтом.', 'images/img/cars/car1.jpg', datetime('now','-5 day')),
  ('Команда тижня: волонтери Вінниці', 'Знайомтесь із людьми, які щодня обробляють десятки запитів на допомогу.', 'Розмова з волонтерами, що координують запити з регіонів. Вони діляться досвідом і розповідають, як ефективно розподіляти ресурси.', 'images/img/org/media1.jpg', datetime('now','-2 day'));
